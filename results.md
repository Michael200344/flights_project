...
IDDFS Algorithm:
 ...
 3) getNumFlightsRequired, with function signature: int getNumFlightsRequired(), is just a helper function to retrieve the minimum number of flights required to reach the destination airport from the starting airport. This is important because the user not only wants to know if travel between the two airports is possible within a max number of flights, but they also want to know exactly how many flights it will take.
